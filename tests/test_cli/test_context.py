...
def test_build_context_defaults(applypilot_home) -> None:
    ctx = build_context()
    assert ctx.base_url.startswith("http")
    assert ctx.output_format == "human"


def test_build_context_format_json(applypilot_home) -> None:
    ctx = build_context(output_format="json")
    assert ctx.output_format == "json"


def test_build_context_invalid_format_falls_back_human(applypilot_home) -> None:
    ctx = build_context(output_format="xml")
    assert ctx.output_format == "human"


def test_build_context_base_url_override(applypilot_home) -> None:
    ctx = build_context(base_url="http://custom:9000/")
    assert ctx.base_url == "http://custom:9000"


def test_build_context_loads_saved_config(applypilot_home) -> None:
...
def test_build_context_loads_credentials(applypilot_home, write_credentials) -> None:
